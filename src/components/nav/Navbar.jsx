          ...
          {isMenuClose ? (
            ...
            <div className=" menu-content bg-[#FFFFFF0A] backdrop-blur-lg fixed z-10 top-0  right-0 h-screen ">
              <div className="pl-44 pt-4">
                <img
                  src="/assets/shared/icon-hamburger-close.svg"
                  alt="menu-icon"
              ...
              <ul className="menus  text-white flex flex-col space-y-6 pt-20  pl-8  pr-20 ">
                {/* {Menu.map((list, index) => (
                  <Link
                    to={`/${list.lists}`}
                    key={index}
                    className="flex flex-row space-x-2 text-[16px] font-Barlow leading-5  cursor-pointer transition-all duration-300 ease-in-out hover:text-blue-500 hover:scale-105"
                  >
        ...
        <ul className="navbars hidden md:flex text-white md:text-sm  flex-row space-x-6  py-6 pr-8  md:bg-opacity-10 md:bg-[#FFFFFF0A] md:backdrop-blur-sm lg:backdrop-blur-lg lg:pl-20  lg:pr-40">
          {Menu.map((list, index) => (
            <Link
              to={`/${list.lists}`}
              key={index}
              className="flex flex-row space-x-2 cursor-pointer transition-all duration-300 ease-in-out hover:text-blue-500 hover:scale-105"
            >
              <a className="flex flex-row  space-x-2 cursor-pointer">
